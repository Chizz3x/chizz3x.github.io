...
export abstract class CommandBase {
  ...
  static traverseArgs(
    ...
    if (argDef.subArgs) {
      for (const subArg of argDef.subArgs) {
        if (subArg.name === token || subArg.alts?.includes(token)) {
          return this.traverseArgs(subArg, tokens.slice(1), context);
        }
      }
    }
  ...
  execute(props: NCommandBase.IExecuteProps) {
    const tokens = CommandBase.parseArgStr(props.argStr);
    return CommandBase.traverseArgs(this.args, tokens, props);
  }
}
...
export namespace NCommandBase {
  export interface IArg {
    ...
    alts?: string[];
    subArgs?: IArg[];
    execute: (args: string[], context: IExecuteProps) => TAction | null;
  }
  export type TAction =
    | {
        name: 'clear';
